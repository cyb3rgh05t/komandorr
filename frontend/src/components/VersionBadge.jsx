...
function VersionBadge() {
  ...
  const checkVersion = async () => {
    try {
      ...
      // Fallback to release.txt if API is not available
      try {
        const response = await fetch("/release.txt");
        const text = await response.text();
        const versionText = text.trim();
        if (versionText) {
          const cleanVersion = versionText.startsWith("v")
        ...
        console.error("Error checking version:", fallbackError);
      }
    }
  };
